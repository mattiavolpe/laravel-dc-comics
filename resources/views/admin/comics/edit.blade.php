...
<div class="container py-5">
  <h1 class="text-center text-light mb-5">Editing "{{ $comic -> title }}" comic</h1>
  <form action="{{ route('comics.update', $comic->id) }}" method="post" class="text-light">
    @csrf
    @method("put")
    <div class="mb-3">
      <label for="title" class="form-label">Title</label>
      <input type="text" name="title" id="title" class="form-control" placeholder="Comic title here..." aria-describedby="titleHelper" value="{{ $comic -> title }}">
      <small id="titleHelper" class="text-muted">Insert comic title</small>
    </div>
    <div class="mb-3">
      <label for="description" class="form-label">Description</label>
      <textarea class="form-control" name="description" id="description" rows="3" placeholder="Comic description here...">{{ $comic -> description }}</textarea>
      <small id="descriptionHelper" class="text-muted">Insert comic description</small>
    </div>
    <div class="mb-3 d-flex align-items-center gap-5">
      <img width="100" src="{{ $comic -> thumb}}" alt="{{ $comic -> title }} cover image">
      <div class="flex-grow-1">
        <label for="thumb" class="form-label">Cover image</label>
        <input type="text" name="thumb" id="thumb" class="form-control" placeholder="Comic cover image path here..." aria-describedby="thumbHelper" value="{{ $comic -> thumb }}">
    ...
    <div class="mb-3">
      ...
      <input type="text" name="series" id="series" class="form-control" placeholder="Comic series here..." aria-describedby="seriesHelper" value="{{ $comic -> series }}">
      <small id="seriesHelper" class="text-muted">Insert comic series</small>
    </div>
    <div class="mb-3">
      <label for="sale_date" class="form-label">Sale date</label>
      <input type="date" name="sale_date" id="sale_date" class="form-control" placeholder="Comic sale date here..." aria-describedby="sale_dateHelper" value="{{ $comic -> sale_date }}">
